...
function validate( opts, options ) {
	...
	if ( options.hasOwnProperty( 'a' ) ) {
		opts.a = options.a;
		if ( !isNumber( opts.a ) ) {
			return new TypeError( 'cdf()::invalid option. `a` parameter must be a number primitive. Option: `' + opts.a + '`.' );
		}
	}
	if ( options.hasOwnProperty( 'b' ) ) {
		opts.b = options.b;
		if ( !isNumber( opts.b ) || opts.b <= ( opts.a || 0 ) ) {
			return new TypeError( 'cdf()::invalid option. `b` parameter must be a number primitive greater than `a`. Option: `' + opts.b + '`.' );
		}
	}
	if ( options.hasOwnProperty( 'c' ) ) {
		opts.c = options.c;
		if ( !isNumber( opts.c ) || opts.c < ( opts.a || 0 ) || opts.c > ( opts.b || 0 ) ) {
			return new TypeError( 'cdf()::invalid option. `c` parameter must be a number primitive in the interval `[a,b]`. Option: `' + opts.c + '`.' );
		}
	}
