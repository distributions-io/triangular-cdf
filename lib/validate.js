...
function validate( opts, options ) {
	...
	if ( options.hasOwnProperty( 'b' ) ) {
		opts.b = options.b;
		if ( !isNumber( opts.b ) ) {
			return new TypeError( 'cdf()::invalid option. `b` parameter must be a number primitive. Option: `' + opts.b + '`.' );
		}
	}
	if ( options.hasOwnProperty( 'c' ) ) {
		opts.c = options.c;
		if ( !isNumber( opts.c ) ) {
			return new TypeError( 'cdf()::invalid option. `c` parameter must be a number primitive. Option: `' + opts.c + '`.' );
		}
	}
